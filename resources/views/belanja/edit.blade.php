<x-app-layout>
  ...
  <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
    ...
    <div class="grid grid-cols-1">
      <!-- Breadcrumb Start -->
      <div x-data="{ 
        pageName: `Edit Belanja`,
        urls:[
          {name: 'Belanja', url: '{{ route('belanja') }}'},
        ]
      }">
      ...
      >
        ...
        <div class="grid grid-cols-12 border border-gray-100 dark:border-gray-800">
          <form
            ...
            action="{{ route('belanja.update',$konsumsi->id) }}"
            method="POST"
          >
            @csrf
            @method('PUT')
            ...
            <div>
              ...
              <div class="relative">
                <input
                  type="date"
                  name="tanggal"
                  value="{{ old('tanggal',$konsumsi->tanggal) }}"
              ...
              >
                ...
                >
                  ...
                  @foreach ($akuns as $akun)
                    <option
                    value="{{ $akun->id }}"
                    @if (old('debet_id',$konsumsi->debet->id) == $akun->id)
                      selected
                    @endif
                    class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                    >
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  @foreach ($akuns as $akun)
                    <option
                    value="{{ $akun->id }}"
                    @if (old('kredit_id',$konsumsi->kredit->id) == $akun->id)
                      selected
                    @endif
                    class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                    >
             ...
             <div>
              ...
              <input
                type="number"
                name="total"
                value="{{ old('total',$konsumsi->total) }}"
                @error('total')
                  class="dark:bg-dark-900 border-error-300 shadow-theme-xs focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800 w-full rounded-lg border bg-transparent px-4 py-2.5 pr-10 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                @else
                  class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
