<x-app-layout>
  <x-slot name="header">
    {{ __('Edit Pengeluaran Bahan Produksi') }}
  </x-slot>
  ...
  <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
    ...
    <div class="grid grid-cols-1">
      <!-- Breadcrumb Start -->
      <div x-data="{ 
        pageName: `Edit Pengeluaran Bahan Produksi`,
        urls:[
          {name: 'Pengeluaran Bahan Produksi', url: '{{ route('belanja') }}'},
        ]
      }">
      ...
      >
        <div class="px-5 py-4 sm:px-6 sm:py-5">
          ...
          >
            Data Pengeluaran Bahan Produksi
          </h3>
        </div>
        <div class="grid grid-cols-12 border border-gray-100 dark:border-gray-800">
          <form
            ...
            action="{{ route('belanja.update',$belanja->id) }}"
            method="POST"
          >
            @csrf
            @method('PUT')
            ...
            <div>
              ...
              <div class="relative">
                <input
                  type="date"
                  name="tanggal"
                  value="{{ old('tanggal',$belanja->tanggal) }}"
              ...
              >
                ...
                >
                  ...
                  @foreach ($konsumsis as $konsumsi)
                    <option
                    value="{{ $konsumsi->id }}"
                    @if (old('konsumsi_id',$belanja->konsumsi_id) == $konsumsi->id)
                      selected
                    @endif
                    class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                    >
